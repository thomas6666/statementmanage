...
(function($) {
	...
	$.fn.calculator = function(params){
		...
		return this.each(function(){
			...
			if(!calc.hasClass('converted2calc')){
				...
				$(".calc-button").hover(
					  function () {
					    $(this).addClass("calc-btn-hover");
					  },
					  function () {
					    $(this).removeClass("calc-btn-hover");
					  }
				); 
			}
		});
	...
	$.fn.calculator.format = function(calc) {
		return calc.addClass('calculator calc-wrapper ui-widget ui-corner-all ui-widget-content').html('<div class="calc-title-bar-container ui-widget-header" style="height:20px;left:0;position:absolute;right:0;top:0;"><div class="calc-title-bar" style="padding:2px 3px;"></div></div><div class="calc-container" style="position:absolute;left:2px;right:2px;top:24px;bottom:2px"><div class="calc-text-wrapper" style="height:20%;left:3px;position:absolute;right:0;top:3%;">'
			//+'<label class="calc-text" style="background:#fff;border:1px  solid #CCC;border-bottom:0px;bottom:0;left:4%;overflow:hidden;padding:2px;position:absolute;right:4%;top:0;text-align:right;vertical-align:middle;height:24px;"></label>'
			+'<label class="calc-text" style="background:#fff;border:1px solid #CCC;bottom:0;left:4%;overflow:hidden;padding:2px;position:absolute;right:4%;top:0px;text-align:right;vertical-align:middle;height:48px;"></label></div>'
			+'<div class="calc-buttons-wrapper" style="bottom:4px;left:2px;position:absolute;right:2px;top:23%;">'
			+'</div></div>').attr('tabindex',0).css({width:calc.settings.width, height:calc.settings.height, outline:'none', position:'relative', zIndex:99999}).focusin(function(){calc.find('.calc-text').css({backgroundColor:'#FFFFF2'})}).focusout(function(){calc.find('.calc-text').css({backgroundColor:''})});
	};
		...
		input: function(calc, button){
			if(calc._executable){
				if(calc.text.length < calc.settings.accuracy||calc._cleartextflag){
					...
					if(input == '' && calc.expression.indexOf('*') !== -1) {
						input = '0';
					}
					if(input == '0.' && calc.expression.indexOf('*0') !== -1) {
						calc.expression = calc.expression.substr(0,calc.expression.length-1);
					}
				...
				calc._cleartextflag = true;					// clear display on next user input
				calc.expression = (calc.expression=='')?'0'+button.symbol:($.fn.calculator.validate.operation(calc.expression)?calc.expression.replace(/[-+/\*]$/,button.symbol):calc.expression+button.symbol);
			}
		}
	}
	...
	$.fn.calculator.buttons = {
		...
		*/
			96:{symbol:'0', keycode: 48, left:'9px', top:'159px', width:'98px', height:'30px', onPress:function(calc){calc._executable = true;$.fn.calculator.action.input(calc, this);} },
			97:{symbol:'1', keycode: 49, left:'9px', top:'121px', width:'44px', height:'30px', onPress:function(calc){calc._executable = true;$.fn.calculator.action.input(calc, this);} },
			98:{symbol:'2', keycode: 50, left:'62px', top:'121px', width:'44px', height:'30px', onPress:function(calc){calc._executable = true;$.fn.calculator.action.input(calc, this);} },
			99:{symbol:'3', keycode: 51, left:'115px', top:'121px', width:'44px', height:'30px', onPress:function(calc){calc._executable = true;$.fn.calculator.action.input(calc, this);} },
			100:{symbol:'4', keycode: 52, left:'9px', top:'83px', width:'44px', height:'30px', onPress:function(calc){calc._executable = true;$.fn.calculator.action.input(calc, this);}  },
			101:{symbol:'5', keycode: 53, left:'62px', top:'83px', width:'44px', height:'30px', onPress:function(calc){calc._executable = true;$.fn.calculator.action.input(calc, this);}  },
			102:{symbol:'6', keycode: 54, left:'115px', top:'83px', width:'44px', height:'30px', onPress:function(calc){calc._executable = true;$.fn.calculator.action.input(calc, this);}  },
			103:{symbol:'7', keycode: 55, left:'9px', top:'46px', width:'44px', height:'30px', onPress:function(calc){calc._executable = true;$.fn.calculator.action.input(calc, this);}  },
			104:{symbol:'8', keycode: 56, left:'62px', top:'46px', width:'44px', height:'30px', onPress:function(calc){calc._executable = true;$.fn.calculator.action.input(calc, this);}  },
			105:{symbol:'9', keycode: 57, left:'115px', top:'46px', width:'44px', height:'30px', onPress:function(calc){calc._executable = true;$.fn.calculator.action.input(calc, this);}  },
			110:{symbol:'.', keycode: 110, left:'115px', top:'159px', width:'44px', height:'30px', onPress:function(calc){calc._executable = true;$.fn.calculator.action.input(calc, this);} },
			111:{symbol:'/', keycode: 111, left:'9px', top:'9px', width:'44px', height:'30px', onPress:function(calc){calc._executable = false;$.fn.calculator.action.input(calc, this);} },
			106:{symbol:'*', keycode: 106, left:'62px', top:'9px', width:'44px', height:'30px', onPress:function(calc){calc._executable = false;$.fn.calculator.action.input(calc, this);} },
			107:{symbol:'+', keycode: 107, left:'169px', top:'46px', width:'44px',height:'67px', onPress:function(calc){calc._executable = false;$.fn.calculator.action.input(calc, this);} },
			109:{symbol:'-', keycode: 109, left:'115px', top:'9px', width:'44px', height:'30px', onPress:function(calc){calc._executable = false;$.fn.calculator.action.input(calc, this);} },
			13:{symbol:'=', keycode: 13, left:'169px', top:'121px', width:'44px', height:'67px', onPress:function(calc){
				if($.fn.calculator.validate.expression(calc.expression)){
					calc.text = eval(calc.expression);
					calc._lastresult = calc.text + '';
					calc.text = (calc.text.toPrecision(calc.settings.accuracy).indexOf('e') == -1)?parseFloat( calc.text.toPrecision(calc.settings.accuracy)) + '':calc.text.toPrecision(calc.settings.accuracy);
		...
		27:{symbol:'C', keycode: 27, left:'169px', top:'9px', width:'44px', height:'30px', onPress:function(calc){calc._executable = true;calc.expression = '';calc.text='0';calc._lastresult = null;}}
	}
